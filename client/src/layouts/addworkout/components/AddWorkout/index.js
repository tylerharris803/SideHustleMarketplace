...
//   const [workoutName, setWorkoutName] = useState("");
    ...
    const [selectedExercises, setSelectedExercises] = useState(
      Array.from({ length: exerciseCount }, () => ({ exerciseId: "", reps: "", sets: "", duration: "", notes: "" }))
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const { data: workoutResult, error: workoutError } = await supabase.from("workout").upsert([workoutData]).select();
      if (workoutError) {
      ...
      const { data: exerciseResult, error: exerciseError } = await supabase.from("customized_exercise").upsert(exerciseRecords).select();
      if (exerciseError) {
  ...
  return (
    <Card id="workout-form">
      ...
      {selectedExercises.map((exercise,index)=> (
          ...
          <MDBox mb={2}>
            <FormControl fullWidth>
              ...
              <Select sx={{ minHeight:'43px'}}
                ...
                onChange={(event) => handleExerciseChange(index, "exerciseId", event.target.value)}
        ...
        <MDBox component="ul" display="flex" flexDirection="row" justifyContent="right" p={0} m={0}>
          <MDBox mb={2}mr={2}>
          ...
          <MDBox mb={2}mr={2}>
            <TextField label="Duration" variant="outlined" 
              ...
              onChange={(event) =>
                handleExerciseChange(index, "duration", event.target.value)
              }
            />
          </MDBox>
          <MDBox mb={2} mr={2}>
